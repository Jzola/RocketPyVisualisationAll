...
namespace BarGraph.VittorCloud
{
    public class BarGraphManager : GraphBox
    {
        ...
        public void GenerateBarWithAnimTypeTwo(int xIndex, int zIndex, float yValue, float scaleFactor, float animSpeed, int ymin, int xMax, Color barColor, int barID)
        {
            if (barRef == null)
                return;
            ...
            barProperty.SetBarID(barID);

            float yscale = (yValue - ymin) * scaleFactor;
            bar.transform.localScale = new Vector3(bar.transform.localScale.x, yscale, bar.transform.localScale.z);

            // If the number of bars is higher than the length of the x-axis, rescale the size of each bar.
            if (ListOfXPoint.Count > XAxis.transform.localScale.x)
            {
                float xScale = (XAxis.transform.localScale.x / ListOfXPoint.Count);
                bar.transform.localScale = new Vector3(xScale, yscale, bar.transform.localScale.z);
                Vector3 pos1 = bar.GetComponentInChildren<MeshRenderer>().gameObject.transform.localScale;
                bar.GetComponentInChildren<MeshRenderer>().gameObject.transform.localScale = new Vector3(1, pos1.y, pos1.z);
            }

            // Create the bar label
            // **This has to be done last otherwise error occurs due to barmesh not having instantiated**
            barProperty.SetBarLabelVisible(yValue.ToString("0.0"), graphScaleFactor);
            ListOfGroups[zIndex].ListOfBar.Add(bar);
        }

        public void GenerateBarWithAnimTypeTwo(int xIndex, int zIndex, float yValue, float scaleFactor, float animSpeed, int ymin, int xMax, Material barMaterial, int barID)
        {
            if (barRef == null)
                return;
