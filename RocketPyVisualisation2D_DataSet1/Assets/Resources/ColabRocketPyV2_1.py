...
4. You will require the Rocket folder in your google drive. If you do not have these, they can be obtained from https://github.com/RocketPy-Team/RocketPy/releases - You will need version 11, and will need to Rename the file to Rocket.

5. You do not have to read or understand the code as prompts will be provided wherever user input is required.
6. Generated Files can be dropped into the RocketPyVisualisation project folder directory: Assets/Resources/
...
if not os.path.isdir(path):
  os.makedirs(path)
  print(f"You will find input data values in {path}")
else:
...
#V2.2 For compatibility with the new bar graph set ensure that all motors are used.
motorChoice = 'n'
...
varChoice = input()
#if windspeed is chosen, get all values. Note that a negative value blows from opposite direction
...
#V2_1 Debug lines to ensure code running correctly without having to generate the full simulation - OPTIONAL
...
#V2_1 debug line to check source of issue when using 'step' value after user input - OPTIONAL
print(type(step))
...
#V2.2 The default should be 6 again to ensure that datasets conform to requirements.
...
#V2 updated this to use input iterations
#V2.2 reverted to 6 iterations per motor to ensure standardised datasets.
iterations = 6
...
#V2.2 Revert the motorchoice to include all 5 engines.
motorChoice = 'n'
